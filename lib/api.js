...
class Api {
  setup(mode, finalizer) {
    console.log(`JRQ-DEBUG: setup called with mode=${mode}, this.config=${JSON.stringify(this.config, null, '  ')}`);
    this.config = new Config(mode, this.options);
    this.config.setDefaultOptions(this.defaultOptions);
    console.log(`JRQ-DEBUG: this.defaultOptions=${JSON.stringify(this.defaultOptions, null, '  ')}`);
