...
class Config {
  constructor(appMode, progOptions, config) {
    console.log(`JRQ-DEBUG: Config constructor called with appMode=${appMode}, progOptions=${JSON.stringify(progOptions, null, '  ')}, config=${JSON.stringify(config, null, '  ')}`);
    ...
    try {
      console.log(`JRQ-DEBUG: Config constructor finished. this=${JSON.stringify(this, null, '  ')}`);
    }
    catch (e) {
      console.warn('JRQ-DEBUG: Caught exception while trying to print config', e);
    }
  }
  ...
  read(callback) {
    ...
    console.log(`JRQ-DEBUG: read called with configFile=${configFile}, this.progOptions=${JSON.stringify(this.progOptions, null, '  ')}, typeof callback=${typeof callback}`);
  ...
  resolveConfigPath(filepath) {
    console.log(`JRQ-DEBUG: resolveConfigPath called with filepath=${filepath}. this.progOptions.config_dir = ${this.progOptions.config_dir}`);
  ...
  reverseResolvePath(filepath) {
    console.log(`JRQ-DEBUG: reverseResolvePath called with filepath=${filepath}. this.cwd()=${this.cwd()}`);
  ...
  readConfigFile(configFile, callback) {
    console.log(`JRQ-DEBUG: readConfigFile called with configFile=${configFile}. typeof callback=${typeof callback}`);
    if (!configFile) { // allow empty configFile for programmatic setups
      console.log(`JRQ-DEBUG: !configFile (configFile was empty)`);
      if (callback) {
        callback.call(this);
      }
    } else if (configFile.match(/\.js$/)) {
      console.log(`JRQ-DEBUG: configFile has .js extension`);
      this.readJS(configFile, callback);
    } else if (configFile.match(/\.json$/)) {
      console.log(`JRQ-DEBUG: configFile has .json extension`);
      this.readJSON(configFile, callback);
    } else if (configFile.match(/\.yml$/)) {
      console.log(`JRQ-DEBUG: configFile has .yml extension`);
  ...
  readJS(configFile, callback) {
    console.log(`JRQ-DEBUG: readJS called with configFile=${configFile}. typeof callback=${typeof callback}`);
    this.fileOptions = require(this.resolveConfigPath(configFile));
    console.log(`JRQ-DEBUG: got fileOptions=${JSON.stringify(this.fileOptions, null, '  ')}`);
    if (callback) {
      callback.call(this);
    }
  }
  ...
  readYAML(configFile, callback) {
    console.log(`JRQ-DEBUG: readYAML called with configFile=${configFile}. typeof callback=${typeof callback}`);
  ...
  readJSON(configFile, callback) {
    console.log(`JRQ-DEBUG: readJSON called with configFile=${configFile}. typeof callback=${typeof callback}`);
    fs.readFile(configFile, (err, data) => {
      if (!err) {
  ...
  getConfigProperty(key) {
    //console.log(`JRQ-DEBUG: getConfigProperty called with key=${key}.`);
  ...
  get(key) {
    //console.log(`JRQ-DEBUG: get called with key=${key}.`);
  ...
  set(key, value) {
    console.log(`JRQ-DEBUG: set called with key=${key}, value=${value}.`);
    if (!this.config) {
      this.config = {};
    }
